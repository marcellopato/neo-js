...
# -*- coding: utf-8 -*-
"""
Neo CLI — chat with Neo straight from the terminal (or from Termux on your phone!).
...
REPL commands:
    ...
    /reset       resets Neo's conversation context (calls POST /reset on the backend)
    /voz on|off  turns voice on/off (Termux only, via termux-tts-speak)
    /audio       records mic audio and sends it transcribed (Termux only)
    /copiar      copies the last response to the clipboard (Termux)
    /exit        exits the Neo CLI  (or Ctrl+D)

Configuration (via .env or environment variables):
    INTERNAL_API_KEY    internal token (required — same as Neo's .env)
    NEO_BACKEND_URL     backend URL  (default http://127.0.0.1:5000)
    NEO_GEMINI_API_KEY  optional Gemini key (X-Gemini-API-Key header)
    NEO_ASK_PORT        local approval server port (default 3303)
    NEO_VOICE           "on"/"off" — force voice on Termux (default: on on Termux)

Command approvals:
    The local approval server (port 3303 by default) answers the backend's
    POST /ask. On Termux a NOTIFICATION with Yes/No buttons appears; outside
    Termux (or without termux-api) the request is shown in the terminal for
    you to answer yes/no (sim/não also works for compatibility).
"""
...
def notify_ask(command, resp_file):
    """Sends a Termux notification with Yes/No buttons that write to the file."""
    try:
        subprocess.Popen(
            [
                "termux-notification",
                "--id", "neo_ask",
                "--title", "🔐 Neo requests approval",
                "--content", f"Command: {command}",
                "--sound",
                "--button1", "✅ Yes",
                "--button1-action", f"echo YES > {resp_file}",
                "--button2", "❌ No",
                "--button2-action", f"echo NO > {resp_file}",
            ],
...
def wait_decision(resp_file=None, timeout=120):
    """Waits for the user's decision: notification buttons (file) OR typing.
    ...
    while time.time() < deadline:
        ...
        if os.name == "posix":
            ...
            if sys.stdin in ready:
                ...
                if line:
                    return line in ("sim", "s", "yes", "y", "ok", "pode", "aprovar", "approve", "allow")
        else:
...
def request_approval(command):
    """Shows the command and asks for approval. Returns True/False."""
    approval_pending.set()
    try:
        print("\n" + colorize("🔐 " + "=" * 58, C.YELLOW))
        print(colorize("  Neo wants to run this command on the system:", C.YELLOW))
        ...
        if termux_cmd("termux-notification"):
            ...
            print(colorize("  📲 Notification sent! Tap Yes/No or type here.", C.DIM))

        print("  → Approve? [y/N] ", end="", flush=True)
        approved = wait_decision(resp_file)
        print()
        if approved:
            print(colorize("  ✅ Approved!", C.GREEN))
        else:
            print(colorize("  ❌ Denied.", C.RED))
        return approved
    finally:
...
def start_ask_server(port):
    ...
    try:
        ...
        print(
            colorize(
                f"  ⚠️  Approval server not started on port {port}: {e.strerror or e}.",
                C.YELLOW,
            )
        )
        print(
            colorize(
                "     (If the WhatsApp bridge is using the port, that's fine — approvals go through it.)",
                C.DIM,
            )
        )
        return None
    ...
    with requests.post(
        ...
        for raw in r.iter_lines(decode_unicode=True):
            ...
            if "chunk" in data:
                ...
                on_status("💭 Neo is thinking...")
            elif data.get("status") == "executing":
                on_status(data.get("detail") or "⚡ Executing...")
            elif data.get("error"):
...
def reset_conversation(cfg):
    ...
    if r.status_code == 200:
        print(colorize("  🧹 Conversation context reset!", C.GREEN))
    else:
        try:
            detail = r.json().get("detail") or f"HTTP {r.status_code}"
        except Exception:
            detail = r.text[:200] or f"HTTP {r.status_code}"
        print(colorize(f"  ⚠️  Reset failed: {detail}", C.RED))
...
│  /audio         records mic audio and sends it (Termux)      │
│  /copiar        copies the last response (Termux)            │
│  /exit          exits the Neo CLI  (or Ctrl+D)               │
│                                                              │
│  Anything else you type is sent to Neo.                      │
└───────────────────────────────────────────────────────────────┘
""", C.CYAN))
    if is_termux():
        print(colorize("💡 Tip: install termux-api with `pkg install termux-api`\n"
                       "   and the Termux:API app for voice, mic and notifications.", C.DIM))


def print_status(cfg, ask_port):
    lines = [
        ("Backend", cfg.backend),
        ("Internal token", "✅ set" if cfg.token else "❌ MISSING INTERNAL_API_KEY"),
        ("Gemini key", "✅" if cfg.gemini_key else "— (backend's will be used)"),
        ("Mode", "streaming (SSE)" if cfg.stream else "sync"),
        ("Approvals", f"local server :{ask_port}" if ask_port else "disabled"),
        ("Environment", "📱 Termux" if is_termux() else "💻 Terminal"),
...
def record_and_send(cfg):
    ...
    if not termux_cmd("termux-microphone-record"):
        print(
            colorize(
                "  ⚠️  Microphone is only available on Termux with `pkg install termux-api`.",
                C.YELLOW,
            )
        )
        return
    ...
    if os.path.exists(path):
        os.remove(path)
    print(colorize("  🎙️  Recording (up to 15s)... speak now! 🎙️", C.MAGENTA))
    try:
        subprocess.run(
    ...
    if not os.path.exists(path) or os.path.getsize(path) == 0:
        print(colorize("  ⚠️  Nothing recorded. Try again.", C.RED))
        return
    ...
    try:
        ...
        print(colorize(f"  ⚠️  Transcription error: {e}", C.RED))
        return
    if r.status_code != 200:
        print(colorize(f"  ⚠️  Transcription failed: HTTP {r.status_code}", C.RED))
        return
    transcription = (r.json().get("transcription") or "").strip()
    print(colorize(f"  🎙️  Transcription: {transcription}", C.MAGENTA))
    if transcription:
...
def copy_last_response(cfg):
    if not cfg.last_response:
        print(colorize("  ⚠️  No response to copy yet.", C.YELLOW))
        return
    clip = termux_cmd("termux-clipboard-set")
    if not clip:
        print(
            colorize(
                "  ⚠️  Clipboard is only available on Termux (termux-api).",
                C.YELLOW,
            )
        )
        return
    try:
        ...
        print(colorize("  📋 Last response copied to the clipboard!", C.GREEN))
    except Exception as e:
        print(colorize(f"  ⚠️  Copy failed: {e}", C.RED))
    ...
    try:
        if cfg.stream:
            try:
                ...
                print(colorize(f"  ⚠️  Streaming failed ({stream_err}). Falling back to sync mode...", C.YELLOW))
        ...
        print(
            "\n"
            + colorize(
                f"  ⚠️  Backend connection error ({cfg.backend}): {e}", C.RED
            )
        )
        print(colorize("     💡 Is the backend running? Try: ./start.sh (or --no-stream)", C.DIM))
        return
...
def handle_command(cfg, raw):
    ...
    if name in ("/help", "/?"):
        ...
        print(colorize("👋 See you soon! 🧠", C.CYAN))
        sys.exit(0)
    elif name == "/reset":
        ...
        if len(parts) > 1:
            voice_enabled = parts[1].lower() in ("on", "1", "true", "sim", "s", "yes", "y")
        else:
            voice_enabled = not voice_enabled
        if voice_enabled and not termux_cmd("termux-tts-speak"):
            print(colorize("  ⚠️  Voice only works on Termux with termux-api installed.", C.YELLOW))
        print(colorize(f"  🗣️  Voice {'ON' if voice_enabled else 'off'}.", C.GREEN))
    elif name == "/audio":
        record_and_send(cfg)
    elif name == "/copiar":
        copy_last_response(cfg)
    else:
        print(colorize(f"  Unknown command: {name}  (type /help)", C.YELLOW))
...
def parse_args():
    p = argparse.ArgumentParser(description="Neo CLI — chat with Neo from the terminal")
    ...
    p.add_argument("--no-stream", action="store_true", help="use POST /chat without streaming")
    p.add_argument("--no-ask", action="store_true", help="don't start the approval server")
    p.add_argument("--no-voice", action="store_true", help="don't enable voice on Termux")
    return p.parse_args()
...
def build_banner(cfg):
    ...
    content = [
        ...
        ("your agent straight from the terminal", C.CYAN),
        ("", ""),
        ("👋 Hi! I'm Neo.", C.RESET),
        ("Give me tasks, commands and", C.RESET),
        ("questions — I'll run them.", C.RESET),
...
def setup_token(cfg):
    ...
    pedirem de novo.
    """
    print(colorize("\n  🔑 INTERNAL_API_KEY not found in the environment or .env.", C.YELLOW))
    print(colorize("     It's the internal token that protects the backend (same as Neo's .env).", C.DIM))

    if not sys.stdin.isatty():
        print(colorize("  ⚠️  Non-interactive terminal — I won't ask for the key now.", C.RED))
        print(colorize("     Set INTERNAL_API_KEY in .env or export it in the environment before running.", C.DIM))
        return

    try:
        raw = input(colorize("  → Paste the INTERNAL_API_KEY here (or Enter to skip): ", C.CYAN)).strip()
    except (EOFError, KeyboardInterrupt):
        print()
        return
    if not raw:
        print(colorize("  ⚠️  No key — the backend will answer 401. You can set it later in .env.", C.RED))
        return
    ...
    try:
        ...
        if r.status_code == 200:
            verified = True
            print(colorize("  ✅ Token validated! The backend answered OK.", C.GREEN))
        else:
            verified = False
            print(colorize(f"  ⚠️  The backend answered HTTP {r.status_code} — the token looks wrong.", C.YELLOW))
    except requests.exceptions.RequestException as e:
        print(colorize(f"  ⚠️  Couldn't validate (backend offline?): {e}", C.YELLOW))

    if verified is False:
        try:
            retry = input(colorize("  → Save anyway? [y/N] ", C.CYAN)).strip().lower()
        except (EOFError, KeyboardInterrupt):
            retry = ""
        if retry not in ("s", "sim", "y", "yes"):
            print(colorize("  ✋ Not saved. You can retry with the correct key.", C.DIM))
            return
    ...
    try:
        ...
        try:
            ...
            pass
        print(colorize(f"  💾 Saved to {env_path} (permission 600)", C.DIM))
    except OSError as e:
        print(colorize(f"  ⚠️  Couldn't save to .env: {e}", C.RED))
...
def main():
    ...
    if is_termux():
        print(colorize("  📱 Termux detected! Voice, mic and notifications active.", C.GREEN))
        missing = [n for n in ("termux-tts-speak", "termux-notification", "termux-microphone-record") if not termux_cmd(n)]
        if missing:
            print(
                colorize(
                    f"  💡 Missing: {', '.join(missing)} — install `pkg install termux-api` + the Termux:API app.",
                    C.YELLOW,
                )
            )
    ...
    while True:
        ...
        while approval_pending.is_set():
            time.sleep(0.1)
        try:
            raw = input(colorize("💻 you> ", C.GREEN))
        except (EOFError, KeyboardInterrupt):
            print("\n" + colorize("👋 See you soon! 🧠", C.CYAN))
            break
        ...
        try:
            ...
            print("\n" + colorize("  ⏹️  Cancelled.", C.YELLOW))
        except Exception as e:
            print("\n" + colorize(f"  ⚠️  Unexpected error: {e}", C.RED))
